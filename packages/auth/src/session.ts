...
import type { SadoraAuth } from "./neon";
 ...
 */
export async function getCurrentUser(auth: SadoraAuth): Promise<AppUser | null> {
...
export async function requireUser(auth: SadoraAuth): Promise<AppUser> {
...
export async function requireAdmin(auth: SadoraAuth): Promise<AppUser> {
  const user = await requireUser(auth);
